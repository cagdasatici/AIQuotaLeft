...
Cursor support has now been removed from the menu bar, panel, widget payload/configuration, and provider registry. Claude and Codex usage rows are named `5-hour` and `Weekly` (with Claude's optional `Weekly (Sonnet)` row). The floating menu keeps each row on one line using compact window/reset labels such as `5h · 21:16` and `W · Sep 27, 16:16`; when Claude has not started a 5-hour window, it truthfully says `starts on use`. Misleading repeated high-usage sample counts are no longer presented as “limit hits,” and ETA copy clearly marks its result as a pace-based estimate. **45 tests pass** using the installed Python 3.12 environment; `git diff --check` also passes.
