...
Cursor support has now been removed from the menu bar, panel, widget payload/configuration, and provider registry. Claude and Codex usage rows are named `5-hour` and `Weekly` (with Claude's optional `Weekly (Sonnet)` row); every row retains its own absolute local reset timestamp. **42 tests pass** using the installed Python 3.12 environment; `git diff --check` also passes. The unsigned Debug widget-host build succeeds; no GUI or live provider was exercised. Xcode emitted unrelated CoreDevice/CoreSimulator version-mismatch warnings.
